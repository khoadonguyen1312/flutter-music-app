    ...
    try {
      ...
      if (videos != null) {
        for (var video in videos) {
          results.add(Song(video.id.toString(), video.title, null, video.thumbnails.highResUrl));
        }
      }
  ...
  Future<List<Song>> getAllFromPlaylist(String id) async {
    try {
      ...
      await for (var item in playlist) {
        results.add(Song(item.id.toString(), item.title, null, item.thumbnails.highResUrl));
      }
